...
leaflet() %>%
  addTiles() %>%
  addMarkers(data = final_df, 
             lng = ~Longitude, 
             lat = ~Latitude,
             icon = leaflet::makeIcon(iconUrl = "http://leafletjs.com/examples/custom-icons/leaf-green.png"),
             popup = ~paste("Location:", final_df$Location),
             label = ~paste(final_df$Location, "<br>Longitude:", final_df$Longitude, "<br>Latitude:", final_df$Latitude))
